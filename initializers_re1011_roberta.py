    ...
    def __call__(self, module: torch.nn.Module) -> None:
        ...
        for name, parameter in module.named_parameters():
            for initializer_regex, initializer in self._initializers:
                ...
                if name == '_embedder.token_embedder_roberta._matched_embedder.transformer_model.encoder.layer.11.intermediate.dense.weight' or \
                name == '_embedder.token_embedder_roberta._matched_embedder.transformer_model.encoder.layer.10.intermediate.dense.weight':
                    logger.info("#################$$$$$$$$$$$$$%%%%%%%10 and 11 intermediate.dense.weight init")
                    XavierNormalInitializer(name)
                else:
