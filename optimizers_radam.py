...
* [dense_sparse_adam]
* [adamp] (https://github.com/clovaai/AdamP/blob/master/adamp/adamp.py)
...
@Optimizer.register("adamp")
class AdamP(Optimizer, torch.optim.Optimizer):
    """
    NOTE:

    Registered as an `Optimizer` with name "adamp".
...
class RAdam(Optimizer, torch.optim.Optimizer):
    """
    Registered as an `Optimizer` with name "radam".
    """
    ...
    def __init__(self,
        ...
        if isinstance(model_parameters, (list, tuple)) and len(model_parameters) > 0 and isinstance(model_parameters[0], dict):
